          ...
          <form v-else @submit.prevent="submitHandler">
            <div class="form-group input-group">
              ...
              <template v-if="$v.title.$error">
                <div
                  v-if="!$v.title.required || !$v.title.descriptionRegex"
            ...
            <div class="form-group">
              <button :disabled="$v.$invalid" class="btn btn-primary btn-block">Edit Propertie</button>
            </div>
          </form>
        </div>
        <div class="col-lg-4"></div>
...
export default {
  ...
  data: function() {
    return {
      id: store.editedListing._id,
      title: store.editedListing.title,
      city: store.editedListing.city,
      imageUrl: store.editedListing.imageUrl,
      price: store.editedListing.price,
      yearOfConstruction: store.editedListing.yearOfConstruction,
      addressInput: store.editedListing.addressInput,
      descriptionInput: store.editedListing.descriptionInput,
      sizeInput: store.editedListing.sizeInput,
      roomsInput: store.editedListing.roomsInput,
      floorInput: store.editedListing.floorInput,
      creator: store.editedListing.creator,
  ...
  beforeCreate() {
    if (!localStorage.getItem("editedPropertie")) {
      store.editedListing = this.$route.params.selectedListing;
      localStorage.setItem(
        "editedPropertie",
        JSON.stringify(store.editedListing)
      );
    } else {
      store.editedListing = JSON.parse(localStorage.getItem("editedPropertie"));
    }
  ...
  destroyed() {
    ...
    localStorage.removeItem("editedPropertie");
  }
};
</script>
